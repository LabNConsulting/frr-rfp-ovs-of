...
extern int rfapiCliGetRfapiIpAddr(struct vty *, const char *,
...
static const rfp_log_levels_t rfp_log_levels[] = {
	...
	{"emergency", 2, "EMER"}, {"error", 2, "ERR"},
	{"warning", 1, "WARN"},   {"info", 1, "INFO"},
	{"debug", 1, "DBG"},      {NULL, 0, NULL} /* end marker */
};

#define RFP_INIT_LOGLEVEL 0     /* non-zero for debugging */

#define RFI_ACTIVE(rfi) ((rfi) != NULL && (rfi)->listen_ports != NULL)
#define USE_VLANS(rfi, uv) ((uv) > 0 || ((uv) == -1 && (rfi)->use_vlans))
 ...
 *			utility functions
 ***********************************************************************/
static int rfp_do_reset_all(struct rfp_instance_t *rfi)
{
	int ret = CMD_SUCCESS;
...
static uint32_t rfp_get_lni(struct rfp_instance_t *rfi,
...
rfp_flow_mode_t rfp_get_flow_mode(void *parent)
{
...
static int rfp_group_config_search_cb(void *criteria, void *rfp_cfg_group)
{
...
int rfp_get_modes(void *parent, unsigned long long int datapath_id,
		  rfp_flow_mode_t *flow_mode, int *use_vlans)
{
	...
	rgc = rfapi_rfp_get_l2_group_config_ptr_lni(
		rfi, rfp_get_lni(rfi, datapath_id, 0, 0),
		(void *)RFP_GRP_CFG_MODES, rfp_group_config_search_cb);
	/* use default values */
	if (flow_mode)
	...
	if (rgc && rgc->type == RFP_GRP_CFG_MODES) {
		...
		zlog_debug(
			"%s: %016llx using defaults: flow_mode=%d, use_vlans=-1",
			__func__, datapath_id, rfp_get_flow_mode(rfi));

	return ret;
}

static void rfp_reset_group_config_type(struct rfp_group_config *rgc)
{
	...
	case RFP_GRP_CFG_VID:
		...
		break;
	default:
		zlog_warn("%s: unexpected config type (%d)!", __func__,
			  rgc->type);
		break;
	}
	return;
...
static void rfp_set_group_config_type(struct rfp_group_config *rgc,
				      rfp_group_config_type type)
{
	if (rgc == NULL)
		return;
			 ...
			 rfp_group_config_type type)
{
	...
	rgc = rfapi_rfp_get_group_config_ptr_vty(rfi, RFAPI_RFP_CFG_GROUP_L2,
						 vty);
	if (rgc == NULL) {
...
static int rfp_get_loglevel_index(const char *cmd)
{
	int ret = 0;

	while (rfp_log_levels[ret].ncmp != 0
	       && strncmp(cmd, rfp_log_levels[ret].cmd,
			  rfp_log_levels[ret].ncmp)
			  != 0)
		ret++;
...
static const char *rfp_get_loglevel_cmd(int index)
{
	if (index < 0)
		return NULL;
...
static const char *rfp_get_loglevel_ovs(int index)
{
	if (index < 0)
		return NULL;
 ...
 *			CLI/CONFIG
 ***********************************************************************/
extern void rfp_output(void *vvty, const char *string)
{
	struct vty *vty = vvty;
	if (vty == NULL)
		return;
       ...
       "Log all available information           (Level 5)\n")
{
	...
	if (error != NULL) {
		vty_out(vty, "Error setting OpenFlow log level: %s%s", error,
			VTY_NEWLINE);
		return CMD_WARNING;
	}
	rfi->loglevel = ll;
...
ALIAS(vnc_show_rfp_listen_ports, vnc_show_of_listen_ports_cmd,
      "show vnc openflow listen-ports",
      SHOW_STR RFAPI_SHOW_STR OF_SHOW_STR "Display OpenFlow listening ports\n")
       ...
       "Space separated list of TCP port numbers <0-65535>, 0 = disable RFP\n")
{
	...
	if (argc > 16) {
		vty_out(vty, "Only first 16 ports will be added%s",
			VTY_NEWLINE);
		argc = 16;
		return CMD_WARNING;
	}
	...
	for (; argc; --argc, ++argv) {
		...
		VTY_GET_INTEGER_RANGE("Port number", pnum, argv[0]->arg, 0,
				      65535);
		if (pnum == 0)
...
ALIAS(vnc_listen_ports, vnc_of_listen_ports_cmd,
      "vnc openflow listen-ports (0-65535)...", VNC_CONFIG_STR OF_CONFIG_STR
       ...
       SHOW_STR RFAPI_SHOW_STR OF_SHOW_STR "Display OpenFlow VLAN mode\n")
{
	...
	vty_out(vty, "VLAN tags are %s.%s",
		(rfi->use_vlans ? "mapped into network ID (last 12 bits)"
				: "ignored (default)"),
		VTY_NEWLINE);
	return CMD_SUCCESS;
}
       ...
       "Network ID based on VID\n" "Ignore VLAN tags (default)\n")
{
	...
	if (rfi->use_vlans != new &&RFI_ACTIVE(rfi)) {
		...
		vty_out(vty,
			"VLAN mode changed, resetting all running OF state%s",
			VTY_NEWLINE);
       ...
       RFAPI_SHOW_STR OF_SHOW_STR "Display OpenFlow flow entry mode\n")
{
	...
	vty_out(vty, "OpenFlow flow entries are %s.%s",
		(rfi->use_wildcards ? "use wildcards"
				    : "flow specific (default)"),
		VTY_NEWLINE);
	return CMD_SUCCESS;
}
       ...
       "Use wildcards\n" "Use flow specific entries (default)\n")
{
	...
	if (rfi->use_wildcards != new &&RFI_ACTIVE(rfi)) {
		/* config change need to reset all connections */
		vty_out(vty, "Flow mode changed for new connections.%s",
			VTY_NEWLINE);
		vty_out(vty,
...
static int vnc_l2_group_of_dpid_common(struct vty *vty, int argc,
				       ...
				       rfp_group_config_type type)
{
	...
	if (datapath_id == 0) {
		vty_out(vty, "Datapath identifier can't be zero%s",
			VTY_NEWLINE);
		return CMD_WARNING;
	}
	...
	if (argc == 5) {
		if (argv[4]->arg[0] != 'u') /* untagged = 0 */
			VTY_GET_INTEGER_RANGE("VLAN ID", vid, argv[4]->arg, 1,
					      4094);
	...
	if (type == RFP_GRP_CFG_ANY) {		    /* just dpid change */
		if (rgc->type == RFP_GRP_CFG_VID) { /* use old vid */
			type = rgc->type;
			vid = rgc->u.vid;
		} else
	...
	if (RFI_ACTIVE(rfi) && rgc->datapath_id != 0
	    && (rgc->type != type || rgc->datapath_id != datapath_id
       ...
       "Use wildcards\n" "Use flow specific entries\n")
{
	...
	if (RFI_ACTIVE(rfi) && rgc->type == type
	    && rgc->u.modes.use_wildcards != -1
				   ...
				   struct cmd_token **argv, int nve)
{
	...
	/* nve */
	if (nve
	    && (argc == 5 && !rfapiCliGetRfapiIpAddr(vty, argv[4]->arg, &un))) {
		int rc;
		union {
	...
	if (!nve) {
		...
		if (connections)
			rc = rfp_ovs_of_reset_all_by_dpid(rfi->ooi,
							  datapath_id);
		else
			rc = rfp_ovs_of_reset_flow_by_dpid(rfi->ooi,
							   datapath_id);
		vty_out(vty, "Cleared %d NVEs%s", rc, VTY_NEWLINE);
		return CMD_SUCCESS;
	}
...
ALIAS(vnc_rfp_of_reset, vnc_rfp_of_reset_all_cmd,
       ...
       "Space separated list of VIDs <0-4094> (0=untagged)\n")
{
	...
	vty_out(vty, "By default, VLAN tags are %s.%s%s",
		(rfi->use_vlans ? "mapped into network ID (last 12 bits)"
				: "ignored"),
		VTY_NEWLINE, VTY_NEWLINE);
	vty_out(vty, "%-16s %-8s --> %s%s", "OpenFlow DIPD", "VLAN ID",
	...
	for (; argc; --argc, ++argv) {
		...
		vty_out(vty, "%016llx %-8d --> %d (0x%x)%s", datapath_id, vid,
			lni, lni, VTY_NEWLINE);
	}
...
ALIAS(vnc_show_of_switches, vnc_show_of_switches_dpid_cmd,
      "show vnc openflow switches DPID", SHOW_STR RFAPI_SHOW_STR OF_SHOW_STR
      "Display openflow switches information\n"
...
static int rfp_cfg_write_cb(struct vty *vty, void *rfp_start_val)
{
	...
	if (rfi->use_wildcards) {
		vty_out(vty, " vnc openflow flow-mode with-wildcards%s",
			VTY_NEWLINE);
		write++;
	}
 ...
 *    lines written
--------------------------------------------*/
static int rfp_cfg_group_write_cb(struct vty *vty, void *rfp_start_val,
	...
	case RFP_GRP_CFG_MODES:
		...
		if (rgc->u.modes.use_vlans != -1) {
			vty_out(vty, "   openflow vlan-mode %stagged%s",
				(rgc->u.modes.use_vlans ? "" : "un"),
				VTY_NEWLINE);
			write++;
		}
		if (rgc->u.modes.use_wildcards != -1) {
			vty_out(vty, "   openflow flow-mode %s%s",
				(rgc->u.modes.use_wildcards ? "with-wildcards"
							    : "flow-specific"),
				VTY_NEWLINE);
			write++;
		}
 ...
 *
--------------------------------------------*/
static const char *synth_argv[] = {"rfp_ovs_of", /* not counted in argc */
				   "ptcp:"};
		...
		struct rfapi_rfp_cb_methods **cbmp)
{
	...
	global_rfi.ooi =
		rfp_ovs_of_init(&global_rfi, synth_argc, (char **)synth_argv,
				rfp_get_loglevel_ovs(global_rfi.loglevel));
...
static int rfp_fd_accept(struct thread *thread)
{
...
void rfp_fd_listen(void *parent, int fd, rfp_fd_cb_t *cb, void *data)
{
	struct rfp_instance_t *rfi = parent;
	struct rfp_thread_list *thread_list;
	assert(parent != NULL);
...
void *rfp_switch_open(void *parent, struct sockaddr_storage *ss, void *data)
{
	struct rfp_instance_t *rfi = parent;
	void *rfd = NULL;
...
int rfp_mac_add_drop(void *parent, void *rfd, int add,
		     const struct ethaddr *mac, struct rfapi_ip_prefix *prefix,
...
int rfp_mac_lookup(void *parent, void *rfd, const struct ethaddr *mac,
		   struct rfapi_ip_addr *target,
		   unsigned long long int datapath_id, int use_vlans,
		   uint16_t vid, uint32_t in_port, uint32_t port_list_size,
 ...
 */
static int rfp_get_ports_by_single_group(void *parent, void *rfd,
					 unsigned long long int datapath_id,
					 int use_vlans, uint16_t vid,
					 uint32_t in_port,
					 ...
					 struct rfp_ovs_of_out port_list[])
{
	...
	ll = bgp_rfapi_get_labellist_by_lni_label(
		bgp, rfp_get_lni(parent, datapath_id, use_vlans, vid), in_port);
	if (ll != NULL) {
		...
		zlog_debug("%s: have labellist, pcount=%d, last=%u/%u",
			   __func__, pcount, port_list[pcount - 1].vid,
			   port_list[pcount - 1].port);
	} else
		zlog_debug("%s: no labellist, %016llx, vid=%d", __func__,
			   datapath_id, vid);
	return pcount;
}

int rfp_get_ports_by_group(void *parent, void *rfd,
			   unsigned long long int datapath_id, int use_vlans,
			   ...
			   struct rfp_ovs_of_out port_list[])
{
	...
	in_port &= 0xffff;
	bgp = bgp_get_default(); /* assume 1 instance for now */
	rfg = bgp_rfapi_get_group_by_lni_label(
		bgp, rfp_get_lni(parent, datapath_id, use_vlans, vid), in_port);
	if (rfg == NULL || rfg->rt_import_list == NULL)
		return rfp_get_ports_by_single_group(parent, rfd, datapath_id,
						     use_vlans, vid, in_port,
						     port_list_size, port_list);
	...
	for (ALL_LIST_ELEMENTS_RO(bgp->rfapi_cfg->l2_groups, node, rfg))
		if (rt_import_list == rfg->rt_import_list
		    || (rfg->rt_import_list
			&& ecommunity_cmp(rt_import_list,
					  rfg->rt_import_list))) {
			struct listnode *lnode;
			void *data;
			uint16_t out_vid =
				(uint16_t)(rfg->logical_net_id & vmask);
 ...
 */
int rfp_get_ports_by_datapath_id(void *parent,
				 unsigned long long int datapath_id,
				 ...
				 struct rfp_ovs_of_out port_list[])
{
	...
	if (bgp->rfapi_cfg->l2_groups != NULL) {
		...
		mask = ~vmask;
		logical_net_id =
			rfp_get_lni(parent, datapath_id, use_vlans, 0) & mask;
	...
	if (pcount == 0) { /* no matching l2 groups / ports found */
