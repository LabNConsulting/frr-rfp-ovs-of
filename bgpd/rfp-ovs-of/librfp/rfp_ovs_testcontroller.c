...
/* derrived from
 * https://github.com/openvswitch/ovs/blob/master/utilities/ovs-testcontroller.c
 */
...
static int32_t flow_mode_to_wildcards(rfp_flow_mode_t flow_mode)
{
...
static void new_switch(struct switch_ *sw, struct vconn *vconn)
{
...
static void add_port_queue(char *s)
{
	char *save_ptr = NULL;
	char *port_name;
	char *queue_id;
	...
	if (!queue_id) {
		ovs_fatal(0,
			  "argument to -Q or --port-queue should take the form "
			  "\"<port-name>:<queue-id>\"");
	}

	if (!simap_put(&port_queues, port_name, atoi(queue_id))) {
		ovs_fatal(0,
			  "<port-name> arguments for -Q or --port-queue must "
			  "be unique");
	}
}
...
static void parse_options(int argc, char *argv[])
{
	...
	for (;;) {
		...
		case 'w':
			wildcards =
				optarg ? strtol(optarg, NULL, 16) : UINT32_MAX;
			break;
		...
		case OPT_MAX_IDLE:
			if (!strcmp(optarg, "permanent")) {
				...
				if (max_idle < 1 || max_idle > 65535) {
					ovs_fatal(
						0,
						"--max-idle argument must be between 1 and "
		...
		case OPT_WITH_FLOWS:
			error = parse_ofp_flow_mod_file(
				optarg, OFPFC_ADD, &default_flows,
				&n_default_flows, &usable_protocols);
			if (error) {
				ovs_fatal(0, "%s", error);
			}
			break;
		...
		case 'h':
			...
			VLOG_OPTION_HANDLERS OFP_VERSION_OPTION_HANDLERS
				DAEMON_OPTION_HANDLERS
					STREAM_SSL_OPTION_HANDLERS case OPT_PEER_CA_CERT
	...
	if (!simap_is_empty(&port_queues) || default_queue != UINT32_MAX) {
		if (action_normal) {
			ovs_error(
				0,
				"queue IDs are incompatible with -N or --normal; "
		...
		if (!learn_macs) {
			ovs_error(
				0,
				"queue IDs are incompatible with -H or --hub; "
				"not acting as hub");
			learn_macs = true;
		}
	}
...
static int rfp_ss_same(struct sockaddr_storage *a, struct sockaddr_storage *b)
{
	...
	if (a->ss_family == AF_INET6) {
		const struct sockaddr_in6 *sin6a =
			(const struct sockaddr_in6 *)a;
		const struct sockaddr_in6 *sin6b =
...
static int rfp_ooi_and_switch_by_lswitch(struct lswitch *lswitch,
					 struct ovs_of_info **ooip,
					 struct switch_ **swp)
{
	struct switch_ *sw;
	if (lswitch == NULL || ooip == NULL || swp == NULL
	    || global_ooi == NULL)
		return 1;
	*ooip = global_ooi;
...
static void rfp_ooi_switch_add(struct ovs_of_info *ooi, struct switch_ *sw)
{
...
static void rfp_ooi_switch_drop(struct ovs_of_info *ooi, struct switch_ *sw)
{
...
static struct switch_ *rfp_ooi_switch_drop_by_rfd(struct ovs_of_info *ooi,
						  void *rfd)
{
	struct switch_ *sw;
	struct switch_ **this = &ooi->switches;
...
static void rfp_ovs_of_fd_unix(void *vooi, int fd, void *data)
{
...
static void rfp_ovs_of_clear_flows(struct ovs_of_info *ooi, struct switch_ *sw)
{
	lswitch_init_flows(sw->lswitch);
	// TBD -- local flow tracking
}

static void rfp_ovs_of_reset_switch(struct ovs_of_info *ooi, struct switch_ *sw)
{
...
static void rfp_ovs_of_fd_switch(void *vooi, int fd, void *data)
{
	struct ovs_of_info *ooi = vooi;
...
static void rfp_ovs_of_fd_listener(void *vooi, int fd, void *data)
{
	struct ovs_of_info *ooi = vooi;
...
int rfp_ovs_of_listen(void *vooi, uint16_t port)
{
	struct ovs_of_info *ooi = vooi;
	int retval;
	struct vconn *vconn;
	...
	allowed_versions =
		(port == OFP_OLD_PORT ? (1u << OFP10_VERSION)
				      : /* force OF1.0 for old port */
			 get_allowed_ofp_versions());
	retval = vconn_open(name, allowed_versions, DSCP_DEFAULT, &vconn);
	if (!retval) {
		rfp_ovs_of_switch_add(ooi, vconn);
	} else if (retval == EAFNOSUPPORT) {
		struct pvconn *pvconn;
		retval = pvconn_open(name, allowed_versions, DSCP_DEFAULT,
				     &pvconn);
		if (!retval) {
...
int rfp_ovs_of_close_all(void *vooi)
{
	struct ovs_of_info *ooi = vooi;
	int i;
	...
	for (i = 0; i < ooi->n_listeners; i++) {
		rfp_fd_close(ooi->parent,
			     pvconn_get_stream_fd(ooi->listeners[i]));
...
const char *rfp_ovs_of_set_loglevel(const char *ovs_lvl)
{
	return vlog_set_levels_from_string(ovs_lvl);
}

void *rfp_ovs_of_init(void *parent, int argc, char *argv[], const char *ovs_lvl)
{
...
void rfp_ovs_of_destroy(void *vooi)
{
	struct ovs_of_info *ooi = vooi;
...
void rfp_ovs_of_reset_all(void *vooi)
{
	struct ovs_of_info *ooi = vooi;
...
int rfp_ovs_of_reset_all_by_ss(void *vooi, struct sockaddr_storage *ss)
{
	struct ovs_of_info *ooi = vooi;
...
int rfp_ovs_of_reset_all_by_dpid(void *vooi, unsigned long long int dpid)
{
	struct ovs_of_info *ooi = vooi;
	struct switch_ *sw, *next;
	int ret = 0;
...
void rfp_ovs_of_reset_flows(void *vooi)
{
	struct ovs_of_info *ooi = vooi;
	struct switch_ *sw;
...
int rfp_ovs_of_reset_flow_by_ss(void *vooi, struct sockaddr_storage *ss)
{
	struct ovs_of_info *ooi = vooi;
	struct switch_ *sw;
	int ret = 0;
...
int rfp_ovs_of_reset_flow_by_dpid(void *vooi, unsigned long long int dpid)
{
	struct ovs_of_info *ooi = vooi;
	struct switch_ *sw;
	int ret = 0;
 ...
 ***********************************************************************/
static int rfp_ovs_of_port_add_drop(struct ovs_of_info *ooi, struct switch_ *sw,
				    int add, unsigned long long int datapath_id,
				    uint32_t port)
{
	...
	if (!add) { /* drop */
		...
		*pl = pp->next;
		free(pp);
		VLOG_INFO_RL(&rl,
			     "%016llx: dropped port %" PRIu16 " from default",
			     datapath_id, port);
		return 0;
	}
	/* is add */
	...
	if (sw->l2_group_ports_count < 0) { /* needs init */
		sw->l2_group_ports_count = rfp_get_ports_by_datapath_id(
			ooi->parent, datapath_id, sw->use_vlans,
			RFP_OFS_OF_MAX_PORTS, sw->l2_group_ports);
		VLOG_INFO_RL(&rl,
			     "%016llx: initl2_group_ports, count=%d, last=%d",
			     datapath_id, sw->l2_group_ports_count,
			     (sw->l2_group_ports_count
				      ? (int)sw->l2_group_ports
						[sw->l2_group_ports_count - 1]
							.port
				      : -1));
	}
	...
	VLOG_INFO_RL(&rl, "%016llx: adding default port #%d, %d", datapath_id,
		     ++dcount, port);
...
static int rfp_ovs_of_maybe_clone_default_group_list(
	struct ovs_of_info *ooi, struct switch_ *sw, uint32_t in_port,
	uint32_t port_list_size, struct rfp_ovs_of_out *port_list, uint16_t vid)
{
	uint32_t count = 0;
	struct ports_ *pp; /* pointer */
	int in_list = 0;
	...
	VLOG_INFO_RL(&rl, "%016llx: cloned %d, in=%u/%u, in_list=%d", sw->dpid,
		     count, vid, in_port, in_list);
 ...
 ***********************************************************************/
void rfp_ovs_set_sw_features(struct lswitch *lswitch,
			     unsigned long long int datapath_id)
{
	...
	if (rfp_get_modes(ooi->parent, datapath_id, &flow_mode, &use_vlans)) {
		VLOG_DBG(
			...
			datapath_id, sw->use_vlans, sw->flow_mode, use_vlans,
			flow_mode);
		if (sw->flow_mode != flow_mode) {
			sw->flow_mode = flow_mode;
			lswitch_set_wildcards(
				sw->lswitch, flow_mode_to_wildcards(flow_mode));
		}
		sw->use_vlans = use_vlans;
	}
}

int rfp_ovs_of_mac(int add, struct lswitch *lswitch,
		   ...
		   uint32_t lifetime)
{
	...
	VLOG_INFO_RL(&rl, "%016llx: vid=%d " ETH_ADDR_FMT
			  " is on "
			  "port %" PRIu16 " state=%s, life=0x%x",
		     datapath_id, vid, ETH_ADDR_ARGS(*mac), port,
		     (add ? "ADD" : "DROP"), lifetime);
	if (rfp_ooi_and_switch_by_lswitch(lswitch, &ooi, &sw)) {
		VLOG_ERR("lswitch not found!");
		return 0;
	}
	...
	rfp_mac_add_drop(ooi->parent, sw->rfd, add, &rfpmac, &prefix,
			 datapath_id, sw->use_vlans, vid, port, lifetime);

	/* catch and ignore zero macs */
	return 0;
}

int rfp_ovs_of_lookup(struct lswitch *lswitch,
		      unsigned long long int datapath_id, uint16_t vid,
		      const struct eth_addr *dmac, ovs_be32 ipv4_src,
		      ...
		      struct rfp_ovs_of_out port_list[])
{
	...
	VLOG_DBG("%016llx: looking up " ETH_ADDR_FMT
		 " in on "
		 "port %" PRIu16,
		 datapath_id, ETH_ADDR_ARGS(*dmac), in_port);
	...
	if ((dmac->ea[0] & 1) == 0) { /* unicast */
		...
		pcount = rfp_mac_lookup(ooi->parent, sw->rfd, &rfpmac, &addr,
					datapath_id, sw->use_vlans, vid,
					in_port, port_list_size, port_list);
	}
	if (pcount == 0) /* flooding when using l2 groups */
		pcount = rfp_get_ports_by_group(
			ooi->parent, sw->rfd, datapath_id, sw->use_vlans, vid,
			in_port, port_list_size, port_list);
	if (pcount == 0 &&		   /* clone default list? */
	    (!sw->use_vlans ||		   /* ignore vid if not in vid mode */
	     (sw->use_vlans && vid == 0))) /* if vid mod and untagged */
		pcount = rfp_ovs_of_maybe_clone_default_group_list(
			ooi, sw, in_port, port_list_size, port_list, vid);
	if (pcount == 0) /* still 0 = drop */
	{
		port_list[pcount].vid = vid;
		port_list[pcount++].port = OFPP_NONE;
	}
	VLOG_INFO_RL(&rl, "%016llx: vid=%d " ETH_ADDR_FMT " port %" PRIu16
			  " --> %u/%u",
	...
	if (pcount > 1) {
		...
		while (i++ < pcount)
			VLOG_INFO_RL(&rl, "%016llx: vid=%d " ETH_ADDR_FMT
					  " port %" PRIu16 " --> %u/%u, #%d",
				     datapath_id, vid, ETH_ADDR_ARGS(*dmac),
				     in_port, port_list[(i - 1)].vid,
				     port_list[(i - 1)].port, i);
	}
	return pcount;
}

int rfp_ovs_of_switch_close(void *vooi, void *rfd, int reason)
{
	struct ovs_of_info *ooi = vooi;
	struct switch_ *sw;
...
void rfp_ovs_of_show_switches(void *vty, void *vooi,
			      unsigned long long int dpid)
{
	struct ovs_of_info *ooi = vooi;
	struct switch_ *sw;
	...
	for (sw = ooi->switches; sw != NULL; sw = sw->next)
		if (dpid == 0 || dpid == sw->dpid) {
			char tmp[4096];
			sprintf(tmp,
				"Switch DPID:\t\t%016llx\t(FD=%-3d RFD=0x%lx)",
				sw->dpid, sw->fd, (long unsigned int)sw->rfd);
			rfp_output(vty, tmp);
			sprintf(tmp, "VLANS:\t\t%s",
				(sw->use_vlans ? "Used" : "Ignored"));
			rfp_output(vty, tmp);
			sprintf(tmp, "Flowsmode:\t%s",
				(sw->flow_mode == RFP_FLOW_MODE_WILDCARDS
					 ? "Wildcarded"
					 : "Specific"));
			rfp_output(vty, tmp);
			if (sw->default_group_ports == NULL) {
				...
				sprintf(tmp, "Default group ports:\t%u",
					port->port);
				rfp_output(vty, tmp);
			...
			if (sw->l2_group_ports_count == 0) {
				...
				rfp_get_l2_group_str_by_pnum(tmp2, ooi->parent,
							     sw->dpid,
							     sw->use_vlans, i);
				sprintf(tmp, "L2 group ports:\t%u\t%s",
					sw->l2_group_ports[i].port, tmp2);
				rfp_output(vty, tmp);
				while (++i < sw->l2_group_ports_count) {
					rfp_get_l2_group_str_by_pnum(
						tmp2, ooi->parent, sw->dpid,
						sw->use_vlans, i);
					sprintf(tmp, "              \t%u\t%s",
						sw->l2_group_ports[i].port,
						tmp2);
					rfp_output(vty, tmp);
				}
			}
