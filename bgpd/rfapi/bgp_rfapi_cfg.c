       ...
       "use the low-order octet of the NVE's VN address\n")
{
	...
	if (!strcmp(argv[1]->arg, "auto-vn")) {
		...
		value = value_l & 0xff;
		if (!argv[1]->arg[0] || *end) {
			vty_out(vty, "%% Malformed l2 nve ID \"%s\"%s",
       ...
       "Route Origin Extended Community Local Admin Field\n" "Field value\n")
{
	...
	if (localadmin > 0xffff) {
		vty_out(vty, "%% Value out of range (0-%d)%s", 0xffff,
			VTY_NEWLINE);
		return CMD_WARNING;
	}
       ...
       "Allow lifetime to never expire\n")
{
	...
	if (!strcmp(argv[3]->arg, "infinite")) {
		...
		VTY_GET_INTEGER("Response Lifetime",
				bgp->rfapi_cfg->redist_lifetime, argv[3]->arg);
	}
       ...
       "IPv6 routes\n" "Prefix-list for filtering redistributed routes\n")
{
	...
	if (!listnode_lookup(bgp->rfapi_cfg->nve_groups_sequential, rfg)) {
		/* Not in list anymore */
		vty_out(vty, "Current NVE group no longer exists%s",
			VTY_NEWLINE);
		return CMD_WARNING;
	}
       ...
       VNC_CONFIG_STR "Configure a NVE group\n" "Group name\n")
{
	...
	if (!rfg) {
		...
		if (!rfg) {
			/* Error out of memory */
			vty_out(vty, "Can't allocate memory for NVE group%s",
				VTY_NEWLINE);
			return CMD_WARNING;
		}
	...
	if (orphaned_nves) {
		...
		for (ALL_LIST_ELEMENTS(orphaned_nves, node, nnode, rfd)) {
			...
			if (!rc) {
				...
				if (vty)
					vty_out(vty, " to new group \"%s\"%s",
						rfd->rfg->name, VTY_NEWLINE);
			}
		}
				 ...
				 rfapi_group_cfg_type_t type) /* _MAX = any */
{
	...
	if (rfg_name) {
		...
		if (!rfg) {
			if (vty)
				vty_out(vty, "No NVE group named \"%s\"%s",
					rfg_name, VTY_NEWLINE);
			return CMD_WARNING;
		}
	}
       ...
       "IPv6 prefix\n")
{
	...
	if (!str2prefix(argv[2]->arg, &p)) {
		vty_out(vty, "Malformed prefix \"%s\"%s", argv[2]->arg,
			VTY_NEWLINE);
		return CMD_WARNING;
	}
       ...
       "Label Value <0-1048575>\n")
{
	...
	VTY_GET_INTEGER_RANGE("Label value", label, argv[1]->arg, 0,
			      MPLS_LABEL_MAX);
       ...
       "Remove VRF default label\n")
{
	...
	if (!listnode_lookup(bgp->rfapi_cfg->nve_groups_sequential, rfg)) {
		/* Not in list anymore */
		vty_out(vty, "Current VRF group no longer exists%s",
			VTY_NEWLINE);
		return CMD_WARNING;
	}
       ...
       VNC_CONFIG_STR "Configure a L2 group\n" "Group name\n")
{
	...
	if (!rfg) {
		...
		if (!rfg) {
			/* Error out of memory */
			vty_out(vty, "Can't allocate memory for L2 group%s",
				VTY_NEWLINE);
			return CMD_WARNING;
		}
				...
				const char *rfg_name) /* NULL = any */
{
	...
	if (rfg_name) {
		...
		if (!rfg) {
			if (vty)
				vty_out(vty, "No L2 group named \"%s\"%s",
					rfg_name, VTY_NEWLINE);
			return CMD_WARNING;
		}
	}
       ...
       "value\n")
{
	...
	if (!listnode_lookup(bgp->rfapi_cfg->l2_groups, rfg)) {
		/* Not in list anymore */
		vty_out(vty, "Current L2 group no longer exists%s",
			VTY_NEWLINE);
		return CMD_WARNING;
	}

	VTY_GET_INTEGER("logical-network-id", rfg->logical_net_id,
			argv[1]->arg);
       ...
       "Space separated list of label values <0-1048575>\n")
{
	...
	for (; argc; --argc, ++argv) {
		uint32_t label;
		VTY_GET_INTEGER_RANGE("Label value", label, argv[0]->arg, 0,
				      MPLS_LABEL_MAX);
       ...
       "Space separated list of label values <0-1048575>\n")
{
	...
	ll = rfg->labels;
	if (ll == NULL) {
		vty_out(vty, "Label no longer associated with group%s",
			VTY_NEWLINE);
		return CMD_WARNING;
	}
...
int bgp_rfapi_cfg_write(struct vty *vty, struct bgp *bgp)
{
	...
	for (ALL_LIST_ELEMENTS(hc->nve_groups_sequential, node, nnode, rfg))
		if (rfg->type == RFAPI_GROUP_CFG_VRF) {
			++write;
			vty_out(vty, " vrf-policy %s%s", rfg->name,
				VTY_NEWLINE);
			if (rfg->label <= MPLS_LABEL_MAX) {
				vty_out(vty, "  label %u%s", rfg->label,
					VTY_NEWLINE);
			}
			...
			if (rfg->rd.prefixlen) {
				...
				if (AF_UNIX == rfg->rd.family) {
					...
					vty_out(vty, "  rd auto:nh:%d%s", value,
						VTY_NEWLINE);
					...
					if (!prefix_rd2str(&rfg->rd, buf,
						...
						vty_out(vty,
							"!Error: Can't convert rd%s",
							VTY_NEWLINE);
					} else {
						vty_out(vty, "  rd %s%s", buf,
							VTY_NEWLINE);
					}
				}
			}
	...
	if (hc->flags & BGP_VNC_CONFIG_ADV_UN_METHOD_ENCAP) {
		vty_out(vty, " vnc advertise-un-method encap-safi%s",
			VTY_NEWLINE);
		write++;
	}
	...
	{ /* was based on listen ports */
		...
		if (hc->l2_groups) {
			...
			for (ALL_LIST_ELEMENTS_RO(hc->l2_groups, gnode, rfg)) {
				...
				void *data;
				++write;
				vty_out(vty, " vnc l2-group %s%s", rfg->name,
					VTY_NEWLINE);
				if (rfg->logical_net_id != 0)
					vty_out(vty,
						"   logical-network-id %u%s",
						rfg->logical_net_id,
						VTY_NEWLINE);
				if (rfg->labels != NULL
		...
		if (hc->default_rd.family || hc->default_response_lifetime
			...
			if (hc->default_rd.prefixlen) {
				...
				if (AF_UNIX == hc->default_rd.family) {
					...
					vty_out(vty, "  rd auto:vn:%d%s", value,
						VTY_NEWLINE);
		...
		for (ALL_LIST_ELEMENTS(hc->nve_groups_sequential, node, nnode,
			...
			if (rfg->type == RFAPI_GROUP_CFG_NVE) {
				++write;
				vty_out(vty, " vnc nve-group %s%s", rfg->name,
					VTY_NEWLINE);
	...
	if (VNC_EXPORT_BGP_GRP_ENABLED(hc)) {
		...
		vty_out(vty, " vnc export bgp mode registering-nve%s",
			VTY_NEWLINE);
	...
	if (VNC_EXPORT_ZEBRA_GRP_ENABLED(hc)) {
		...
		vty_out(vty, " vnc export zebra mode registering-nve%s",
			VTY_NEWLINE);
	}
	...
	if (hc->redist_mode) /* ! default */
	{
		...
		if (s) {
			vty_out(vty, " vnc redistribute mode %s%s", s,
				VTY_NEWLINE);
		}
	}
