		   ...
		   int flags)
{
  ...
  if ((safi == SAFI_MPLS_VPN) && (flags & RFAPI_AHR_SET_PFX_TO_NEXTHOP))
	 ...
	 */
	if (rt_export_list) {
		attr.extra->ecommunity = ecommunity_merge(
			attr.extra->ecommunity, rt_export_list);
	}
  ...
  "IPv6 address\n")
{
	...
	if (!str2prefix(argv[5]->arg, &pfx)) {
		vty_out(vty, "Malformed address \"%s\"%s", argv[5]->arg,
			VTY_NEWLINE);
		return CMD_WARNING;
	}
	if (pfx.family != AF_INET && pfx.family != AF_INET6) {
		vty_out(vty, "Invalid address \"%s\"%s", argv[5]->arg,
			VTY_NEWLINE);
		return CMD_WARNING;
	}
       ...
       "underlay network interface IPv6 address\n")
{
	...
	vty_out(vty, "rfapi_open: status %d, handle %p, lifetime %d%s", rc,
		handle, lifetime, VTY_NEWLINE);
       ...
       "lifetime\n")
{
	...
	if (!str2prefix(argv[8]->arg, &pfx)) {
		vty_out(vty, "Malformed prefix \"%s\"%s", argv[8]->arg,
			VTY_NEWLINE);
		return CMD_WARNING;
	}
	if (pfx.family != AF_INET && pfx.family != AF_INET6) {
		vty_out(vty, "Bad family for prefix \"%s\"%s", argv[8]->arg,
			VTY_NEWLINE);
		return CMD_WARNING;
	}
       ...
       "target MAC addr\n")
{
  ...
  if ((rc = rfapiCliGetRfapiIpAddr (vty, argv[2], &target)))
	...
	if (rfapiStr2EthAddr(argv[10]->arg, &l2o_buf.macaddr)) {
		vty_out(vty, "Bad mac address \"%s\"%s", argv[10]->arg,
			VTY_NEWLINE);
		return CMD_WARNING;
	}

	VTY_GET_INTEGER("Logical Network ID", l2o_buf.logical_net_id,
			argv[8]->arg);
